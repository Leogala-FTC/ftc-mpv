-- FTC MVP - onboarding + password auth helpers (safe to re-run)

-- 1) add a simple flag to detect first login
alter table public.profiles
  add column if not exists onboarded boolean not null default false;

-- 2) onboarding RPC: user chooses role (user/merchant) and completes profile
--    - creates user wallet
--    - if merchant: creates merchant + wallet + staff link + default pricing (15 buttons)
...
begin
  ...
  if p_role = 'merchant' then
    ...
    -- default pricing: EUR -> tokens, no euro shown to user UI, but pricing exists
    for v_i in 1..array_length(v_btns, 1) loop
      v_eur := v_btns[v_i];
      v_tokens := round(v_eur / 0.02, 2); -- 0.02 fixed estimate for clearing UI; tokens are defined by pricing
