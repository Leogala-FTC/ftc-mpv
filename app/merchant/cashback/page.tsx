"use client";

import { useEffect, useState } from "react";
import { getMerchantCashback, setMerchantCashback } from "@/app/actions/settings";

export default function MerchantCashbackPage() {
  const [current, setCurrent] = useState<number>(3);
  const [value, setValue] = useState<number>(3);
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState<{ type: "ok" | "err"; text: string } | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getMerchantCashback().then(({ percent }) => {
      setCurrent(percent);
      setValue(percent);
      setLoading(false);
    });
  }, []);

  async function handleSave() {
    setSaving(true);
    setMsg(null);
    const res = await setMerchantCashback(value);
    if (res.success) {
      setCurrent(value);
      setMsg({ type: "ok", text: `✓ Cashback aggiornato al ${value}% per questa settimana` });
    } else {
      setMsg({ type: "err", text: res.error ?? "Errore" });
    }
    setSaving(false);
  }

  // Calcolo esempio su €100
  const example = Math.floor(100 * 11.7 * (value / 100));

  return (
    <main className="mx-auto max-w-lg px-4 py-8">
      <h1 className="text-2xl font-semibold mb-1">Cashback ai clienti</h1>
      <p className="text-sm text-gray-500 mb-6">
        Imposta la percentuale di cashback che vuoi restituire ai tuoi clienti su ogni acquisto in euro.
        Puoi aggiornarla ogni lunedì — se non la cambi, rimane quella attuale.
      </p>

      {loading ? (
        <div className="h-40 bg-gray-100 animate-pulse rounded-xl" />
      ) : (
        <>
          {/* Saldo corrente */}
          <div className="bg-indigo-50 border border-indigo-200 rounded-xl px-5 py-4 mb-6">
            <p className="text-xs text-indigo-500 font-semibold uppercase tracking-wide mb-1">Cashback attuale</p>
            <p className="text-3xl font-bold text-indigo-700">{current}%</p>
            <p className="text-xs text-indigo-400 mt-1">
              Su ogni €100 → cliente riceve {Math.floor(100 * 11.7 * (current / 100)).toLocaleString()} token cashback
            </p>
          </div>

          {/* Slider */}
          <div className="bg-white border border-gray-200 rounded-xl px-5 py-5 mb-4">
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium text-gray-700">Nuova percentuale</label>
              <span className="text-2xl font-bold text-indigo-600">{value}%</span>
            </div>

            <input
              type="range"
              min={1}
              max={20}
              step={0.5}
              value={value}
              onChange={(e) => setValue(parseFloat(e.target.value))}
              className="w-full accent-indigo-600 mb-3"
            />

            <div className="flex justify-between text-xs text-gray-400 mb-4">
              <span>Min 1%</span>
              <span>Max 20%</span>
            </div>

            {/* Preset rapidi */}
            <div className="flex gap-2 flex-wrap mb-4">
              {[3, 5, 8, 10, 15, 20].map((p) => (
                <button
                  key={p}
                  onClick={() => setValue(p)}
                  className={`px-3 py-1 rounded-full text-xs font-semibold border transition-colors ${
                    value === p
                      ? "bg-indigo-600 text-white border-indigo-600"
                      : "text-gray-600 border-gray-300 hover:border-indigo-300"
                  }`}
                >
                  {p}%
                </button>
              ))}
            </div>

            {/* Anteprima */}
            <div className="bg-gray-50 rounded-lg p-3 text-sm">
              <p className="text-xs text-gray-500 font-medium mb-1">📊 Anteprima su €100 di acquisto</p>
              <div className="flex justify-between">
                <span className="text-gray-600">Cashback cliente</span>
                <span className="font-semibold text-green-700">+{example.toLocaleString()} token</span>
              </div>
              <div className="flex justify-between mt-1">
                <span className="text-gray-600">Pari a</span>
                <span className="text-gray-500">≈ €{(example / 11.7).toFixed(2)}</span>
              </div>
            </div>
          </div>

          {msg && (
            <div className={`rounded-xl px-4 py-3 text-sm mb-4 ${
              msg.type === "ok" ? "bg-green-50 text-green-700 border border-green-200"
                               : "bg-red-50 text-red-700 border border-red-200"
            }`}>
              {msg.text}
            </div>
          )}

          <button
            onClick={handleSave}
            disabled={saving || value === current}
            className="w-full bg-indigo-600 text-white py-3 rounded-xl text-sm font-semibold hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {saving ? "Salvataggio..." : value === current ? "Nessuna modifica" : `Imposta cashback al ${value}%`}
          </button>

          <p className="text-xs text-gray-400 text-center mt-3">
            La modifica è valida per tutti i pagamenti da questo momento in poi.
          </p>
        </>
      )}
    </main>
  );
}
